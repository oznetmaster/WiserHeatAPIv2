			...
			try
				...
				if (ValidateScheduleType (scheduleData))
					...
					if (scheduleData != null)
						{
						Logger.Error ($"{(scheduleData.TryGetValue ("Type", out var type) ? type : TextUnknown)} is an incorrect schedule type for this device. It should be a {ScheduleType} schedule.");
						}
					else
						{
		...
		public async Task<bool> SetScheduleFromWsDataAsync (IDictionary<string, object> scheduleData, CancellationToken cancellationToken = default)
			...
			try
				...
				if (ValidateScheduleType (scheduleData))
					...
					{
					Logger.Error ($"{(scheduleData.TryGetValue ("Type", out var type) ? type : Constants.TextUnknown)} is an incorrect schedule type for this device. It should be a {ScheduleType} schedule.");
					return false;
					}
				}
			catch (Exception ex)
	...
	public class WiserHeatingSchedule (WiserRestController wiserRestController, string scheduleType, IDictionary<string, object> scheduleData,
		...
		protected override List<IDictionary<string, object>> ConvertWiserToYamlDay (string day, object daySchedule, bool replaceSpecialTimes = false, bool genericSetpoint = false)
			...
			if (daySchedule is Dictionary<string, object> dayDict &&
				 dayDict.TryGetValue (TextTime, out var timeObj) && timeObj is List<object> times &&
				 dayDict.TryGetValue (TextDegreesC, out var tempObj) && tempObj is List<object> temps)
				{
				for (var i = 0; i < times.Count; i++)
					...
					scheduleSetPoints.Add (new Dictionary<string, object>
						  {
								{ TextTime, time },
								{ genericSetpoint ? TextSetpoint : TextTemp,
								  WiserTemperatureFunctions.FromWiserTemp(temps[i]) }
						  });
					}
				}

			return [.. scheduleSetPoints.OrderBy (t => t[TextTime].ToString ())];
			}
		...
		protected override object ConvertYamlToWiserDay (List<IDictionary<string, object>>? daySchedule)
			...
			foreach (IDictionary<string, object> item in daySchedule)
				{
				if (item.TryGetValue (TextTime, out var timeValue))
					{
					var time = timeValue.ToString ().Replace (":", "");
					times.Add (time);
					}

				if (item.TryGetValue (TextTemp, out var tempValue) || item.TryGetValue (TextSetpoint, out tempValue))
					{
					var temp = tempValue.ToString ().Equals (TextOff, StringComparison.OrdinalIgnoreCase)
						? TempOff
		...
		protected override object ConvertYamlToWiserDay (List<IDictionary<string, object>>? daySchedule)
			...
			foreach (IDictionary<string, object> entry in daySchedule)
				...
				try
					...
					if ((entry.TryGetValue ("State", out var stateValue) || entry.TryGetValue (TextSetpoint, out stateValue)) &&
						 stateValue.ToString ().TitleCase () == TextOff)
						{
	 ...
	 IDictionary<string, string> sunrises, IDictionary<string, string> sunsets) : WiserSchedule (wiserRestController, scheduleType, scheduleData, sunrises, sunsets)
		...
		protected override List<IDictionary<string, object>> ConvertWiserToYamlDay (string day, object daySchedule, bool replaceSpecialTimes = false, bool genericSetpoint = false)
			...
			if (daySchedule is IDictionary<string, object> dayDict &&
				 dayDict.TryGetValue (TextTime, out var timeObj) && timeObj is List<object> times &&
				 dayDict.TryGetValue (TextLevel, out var levelObj) && levelObj is List<object> levels)
				{
				for (var i = 0; i < times.Count; i++)
					{
					...
					scheduleSetPoints.Add (new Dictionary<string, object>
						  {
								{ TextTime, timeStr },
								{ genericSetpoint ? TextSetpoint : TextLevel, levels[i] }
						  });
					}
				}
