...
namespace WiserHeatApiV2
	...
	public class WiserRoom
		...
		private static string EffectiveHeatingMode (string mode, double temp)
			{
			if (mode.Equals (TextManual, StringComparison.OrdinalIgnoreCase) && temp == Constants.TempOff)
				{
				return WiserHeatingMode.Off.ToString ();
				}
			else if (mode.Equals (TextManual, StringComparison.OrdinalIgnoreCase))
				{
		...
		public string ControlDirection => _data.TryGetValue ("ControlDirection", out var direction) ? direction.ToString () : TextUnknown;
		...
		public double CurrentTargetTemperature => WiserTemperatureFunctions.FromWiserTemp (
			 _data.TryGetValue ("CurrentSetPoint", out var setPoint) ? setPoint : TempMinimum);
		...
		public double CurrentTemperature => WiserTemperatureFunctions.FromWiserTemp (
			 _data.TryGetValue ("CalculatedTemperature", out var temp) ? temp : TempMinimum, "current");
		...
		public string DemandType => _data.TryGetValue ("DemandType", out var type) ? type.ToString () : TextUnknown;
		...
		public double DisplayedSetpoint => WiserTemperatureFunctions.FromWiserTemp (
			 _data.TryGetValue ("DisplayedSetPoint", out var setPoint) ? setPoint : TempMinimum, "current");
		...
		public string HeatingRate => _data.TryGetValue ("HeatingRate", out var rate) ? rate.ToString () : TextUnknown;
		...
		public string HeatingType => _data.TryGetValue ("HeatingType", out var type) ? type.ToString () : TextUnknown;
		...
		public bool IsOverride => _data.TryGetValue ("OverrideType", out var type) &&
			type.ToString () != TextUnknown && type.ToString () != TextNone;
		...
		public bool IsHeating => _data.TryGetValue ("ControlOutputState", out var state) && state.ToString () == TextOn;
		...
		public double ManualTargetTemperature => WiserTemperatureFunctions.FromWiserTemp (
			 _data.TryGetValue ("ManualSetPoint", out var setPoint) ? setPoint : TempMinimum);
		...
		public async Task<bool> SetModeAsync (string value, CancellationToken cancellationToken = default)
			...
			try
				...
				if (mode == WiserHeatingMode.Off)
					{
					_ = await SetManualTemperatureAsync (TempOff, cancellationToken).ConfigureAwait (false);
					}
					...
					if (await SendCommandAsync (new { Mode = WiserHeatingMode.Manual.ToString () }, cancellationToken: cancellationToken).ConfigureAwait (false))
						{
						if (CurrentTargetTemperature == TempOff)
							{
		...
		public string OverrideType => _data.TryGetValue ("OverrideType", out var type) ? type.ToString () : TextNone;
		...
		public double ScheduledTargetTemperature => WiserTemperatureFunctions.FromWiserTemp (
			 _data.TryGetValue ("ScheduledSetPoint", out var setPoint) ? setPoint : TempMinimum);
		...
		public string WindowState => _data.TryGetValue ("WindowState", out var state) ? state.ToString () : TextUnknown;
