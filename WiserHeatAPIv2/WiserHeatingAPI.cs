...
public class WiserAPI : IDisposable
	...
	public async Task<bool> ReadHubDataAsync (CancellationToken cancellationToken = default)
		...
		try
			...
			// Read data from hub
			Dictionary<string, object> newDomainData = await _wiserRestController.GetHubDataAsync (WISER_HUB_DOMAIN.FormatInvariant (_wiserApiConnection.Host), cancellationToken: cancellationToken).ConfigureAwait (false);
			Dictionary<string, object> newNetworkData = await _wiserRestController.GetHubDataAsync (WISER_HUB_NETWORK.FormatInvariant (_wiserApiConnection.Host), cancellationToken: cancellationToken).ConfigureAwait (false);
			Dictionary<string, object> newScheduleData = await _wiserRestController.GetHubDataAsync (WISER_HUB_SCHEDULES.FormatInvariant (_wiserApiConnection.Host), cancellationToken: cancellationToken).ConfigureAwait (false);
			Dictionary<string, object> newOpenthermData = await _wiserRestController.GetHubDataAsync (WISER_HUB_OPENTHERM.FormatInvariant (_wiserApiConnection.Host), raiseForEndpointError: false, cancellationToken: cancellationToken).ConfigureAwait (false);
	...
	private static readonly Dictionary<string, string> _dataClassToEndpoint = new (StringComparer.OrdinalIgnoreCase)
		{
			 { "domain",    RestConstants.WISER_HUB_DOMAIN },
			 { "network",   RestConstants.WISER_HUB_NETWORK },
			 { "schedules", RestConstants.WISER_HUB_SCHEDULES },
			 { "opentherm", RestConstants.WISER_HUB_OPENTHERM }
		};
