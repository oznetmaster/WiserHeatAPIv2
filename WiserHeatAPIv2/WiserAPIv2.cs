...
namespace WiserHeatApiV2
	...
	public static class WiserTemperatureFunctions
		...
		public static double FromWiserTemp (int? temp, string type = "set_heating", WiserUnits units = WiserUnits.Metric)
			...
			if (!temp.HasValue)
				return 0;
			var realTemp = temp >= TempError ? TempMinimum : ValidateTemperature (Math.Round ((double)temp / 10, 1), type);
		...
		private static double ValidateTemperature (double temp, string type = "set_heating") =>
			type switch
				{
					"hotwater" when temp is TempHwOn or TempHwOff => temp,
					"delta" => temp > MaxBoostIncrease ? MaxBoostIncrease : temp,
					"current" => temp < TempOff ? TempMinimum : temp,
					"set_heating" when temp >= TempError => TempMinimum,
					"set_heating" when temp > TempMaximum => TempMaximum,
					"set_heating" when temp is < TempMinimum and not TempOff => TempMinimum,
					_ => temp
					};
	...
	/// <see cref="Constants.RoomstatMinBatteryLevel"/> and <see cref="Constants.RoomstatFullBatteryLevel"/>,
				...
				if (data.TryGetValue ("ProductType", out var productType) && Level != "No Battery")
					...
					if (productType.ToString () == "RoomStat")
						...
						return PercentageClip (
								 (int)Math.Round (
									  (Voltage - RoomstatMinBatteryLevel) /
									  (RoomstatFullBatteryLevel - RoomstatMinBatteryLevel) * 100
								 )
						);
						}
	...
	public class WiserSignalStrength (IDictionary<string, object> data)
		...
		public string DisplayedSignalStrength => data.TryGetValue ("DisplayedSignalStrength", out var strength) ? strength.ToString () : TextUnknown;
	...
	public class WiserCloud (string cloudStatus, Dictionary<string, object> data)
		...
		public string ApiHost => data.TryGetValue ("WiserApiHost", out var host) ? host.ToString () : TextUnknown;
		...
		public string BootstrapApiHost => data.TryGetValue ("BootStrapApiHost", out var host) ? host.ToString () : TextUnknown;
	...
	public class WiserFirmwareUpgradeItem (Dictionary<string, object> data)
		...
		public string Filename => data.TryGetValue ("FirmwareFilename", out var filename) ? filename.ToString () : TextUnknown;
		}
	...
	public class WiserZigbee (Dictionary<string, object> data)
		...
		public string ModuleVersion => data.TryGetValue ("ZigbeeModuleVersion", out var value) ? value.ToString () : TextUnknown;
		...
		public string EUI => data.TryGetValue ("ZigbeeEUI", out var value) ? value.ToString () : TextUnknown;
		}
	...
	public class WiserElectricalLevelDevice (WiserRestController wiserRestController, Dictionary<string, object> data, Dictionary<string, object> deviceTypeData) : WiserDevice (wiserRestController, data, deviceTypeData)
		...
		/// <inheritdoc />
		public override string Model => Data.TryGetValue ("ProductType", out var type) ? type.ToString () : TextUnknown;
		}
