...
These corrections are included in version 1.1.2. Earlier packages do not consistently apply the configured units to heating models.
...
Setting a manual temperature while the room is Off sends that target directly. Off is already represented as Manual mode at the hub; the library does not first restore the scheduled temperature.
